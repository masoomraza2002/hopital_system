...
import { getPatientPreviousRecords, deletePatientPreviousRecord } from '../../api/patientApi';

const PatientPreviousRecords = () => {
    const { user } = useAuth();
    ...
    const fetchRecords = async () => {
        ...
        setError('');
        try {
            const res = await getPatientPreviousRecords(user.token);
            setRecords(res.data);
        } catch (err) {
    ...
    const confirmDelete = async () => {
        if (recordToDelete) {
            ...
            setDeleteSuccess('');
            try {
                await deletePatientPreviousRecord(recordToDelete._id, user.token);
    ...
    return (
        <MainLayout>
            <h1 className="text-3xl font-bold mb-4 text-primary">My Previous Records</h1>
            ...
            <Card title="List of All Past Records">
                {records.length === 0 ? (
                    <p className="text-muted">No previous medical records found.</p>
                ) : (
                            ...
                            <tbody>
                                {records.map((record) => (
                                    <tr key={record._id}>
                                        ...
                                        <td>{record.doctor?.phoneNumber || 'N/A'}</td>
                                        <td>{record.medicinesPrescribed || 'N/A'}</td>
                                        <td>{record.doctorAdvice || 'N/A'}</td>
                                        <td>
                                            {/* Optional: Patient can delete their own records. Be cautious with this functionality. */}
                                            <Button
                                                variant="danger"
                                                size="sm"
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content rounded-lg">
                            ...
                            <div className="modal-body">
                                Are you sure you want to delete this medical record from {new Date(recordToDelete?.appointment?.date).toLocaleDateString()}? This action cannot be undone.
                            </div>
